...
function generateTableData(originalData) {
	...
	for (var i = 0; i < originalData.length; i++) {
		...
		var row = {
			...
			"Virtual price":
				originalRow.purchase.virtual_currency.amount +
				" " +
				originalRow.purchase.virtual_currency.name,
			"Payment price":
				originalRow.payment_details.payment.amount +
				" " +
				originalRow.payment_details.payment.currency,
...
function updateTable() {
	var suitableRows = getSuitableRows(
	...
	function getSuitableRows(columnName, filterableText) {
		...
		if (
			columnName === "#" &&
			isNumeric(filterableText) &&
			+filterableText > 0 &&
			+filterableText <= tableData.length + 1
		) {
			...
			for (var rowNumber = 0; rowNumber < tableData.length; rowNumber++) {
				...
				var isInTable = tableData[rowNumber][columnName]
					.toLowerCase()
					.indexOf(filterableText);
				if (~isInTable) {
...
function sortColumn(columnNumber, type) {
	...
	switch (type) {
		case "number":
			compare = function (rowA, rowB) {
				var numberA = Number(
					rowA.cells[columnNumber].textContent.split(" ")[0],
				);
				var numberB = Number(
					rowB.cells[columnNumber].textContent.split(" ")[0],
				);
	...
	function getClass() {
		var classList =
			thead.firstElementChild.cells[columnNumber].classList.toString();
...
function getPopularPaymentMethods() {
	...
	for (
		...
		transactionNumber < tableData.length;
		transactionNumber++
	) {
