...
// [...wikipediaMap.matchAll(/<path\s+d="([^"]+)"\s+className="([^"]+)"[^>]*>\s*<title>{"([^"]+)"}<\/title>\s*<\/path>/gm)].map(state => ({ d: state[1], code: state[2], name: state[3] }))
...
// [...wikipediaMap.matchAll(/<path\s+d="([^"]+)"\s+className="([^"]+)"[^>]*>/gm)].map(border => ({ d: border[1], stateA: border[2].split('-')[0], stateB: border[2].split('-')[1] }))
